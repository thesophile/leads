...
export default function ClientQuotation() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const d = await api.get(`/transactions/public/quotations/${encodeURIComponent(token)}/`)
  ...
  const d = data
  const scopeHtml = d.proposalScope || ''
  const detailsHtml = d.termsConditions || ''
        ...
        <div className="mx-auto max-w-2xl px-4 py-10">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm text-center">
            ...
            <h1 className="mt-4 text-lg font-bold text-slate-900">
              {d.clientStatus === 'Accepted' ? 'Quotation accepted' : 'Quotation declined'}
            </h1>
          ...
          <div className="rounded-b-2xl border border-t-0 border-slate-200 bg-slate-50/70 shadow-sm">
            ...
            <div className="border-t border-slate-200 bg-white p-5">
              ...
              <p className="mt-1 text-xs text-slate-500 leading-relaxed">
                Review the quotation above, then accept or decline it. You can add a comment for
