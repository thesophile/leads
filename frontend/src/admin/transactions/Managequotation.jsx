...
// Statuses that may be sent directly to the client by a user with the
    ...
    <Layout>
                ...
                {activeMenuQuote.hasProposal ? (
                  <>
                    ...
                    {canSendToClient &&
                      ...
                      SEND_WITHOUT_APPROVAL_STATUSES.includes(activeMenuQuote.status) && (
                        <button
                          type="button"
                          onClick={(e) => handleOpenSendClient(activeMenuQuote, e)}
      ...
      {sendClientOpen &&
        (() => {
          ...
          const sendClientBypass =
            !!sendClientQuote &&
            sendClientQuote.status !== 'Approved' &&
            sendClientQuote.status !== 'Sent to Client'
          return (
        <div
          className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4"
          onClick={(e) => {
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              {sendClientBypass && (
                <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5 text-[11px] text-amber-800">
                  <svg viewBox="0 0 24 24" className="mt-0.5 h-3.5 w-3.5 shrink-0" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                    <path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" />
                    <line x1="12" y1="9" x2="12" y2="13" />
                    <line x1="12" y1="17" x2="12.01" y2="17" />
                  </svg>
                  <span>
                    This proposal has not been approved. You are sending it without approval.
                  </span>
                </div>
              )}
