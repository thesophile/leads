...
export default function RawData() {
  ...
  const [assignSelectedStaffList, setAssignSelectedStaffList] = useState([])
  const [assignSelectedStaffOpen, setAssignSelectedStaffOpen] = useState(false)
  const [assignSelectedSuccessMessage, setAssignSelectedSuccessMessage] = useState('')
  const [discardAssignSelectedOpen, setDiscardAssignSelectedOpen] = useState(false)
  ...
  const { dirty: assignSelectedDirty, reset: resetAssignSelectedDirty } = useDirty(
    assignSelectedOpen,
    useMemo(
      () => ({ assignSelectedStaffList }),
      [assignSelectedStaffList]
    )
  )
  ...
  function requestCloseAssignSelected() {
    if (assignSelectedDirty) setDiscardAssignSelectedOpen(true)
    else setAssignSelectedOpen(false)
  }
...
async function handleBulkImport(e) {
  ...
  function toggleAssignSelectedStaff(name) {
    setAssignSelectedStaffList((prev) =>
      prev.includes(name) ? prev.filter((n) => n !== name) : [...prev, name]
    )
  }

  function toggleAllAssignSelectedStaff() {
    const allNames = assignableStaff.map((s) => s.name)
    const allSelected =
      assignSelectedStaffList.length === allNames.length && allNames.length > 0
    setAssignSelectedStaffList(allSelected ? [] : allNames)
  }

  async function handleExecuteAssignSelected(e) {
    e.preventDefault()
    if (isSaving) return
    setError('')
    if (assignSelectedStaffList.length === 0) {
      setError('Please select at least one staff member.')
      return
    }
    if (selectedIds.size === 0) {
      setError('Please select at least one raw lead to assign.')
      return
    }
    ...
    try {
      const res = await api.post('/transactions/leads/assign/', {
        assigned_to: assignSelectedStaffList,
        lead_ids: [...selectedIds],
      })
      setAssignSelectedSuccessMessage(
        `✓ Successfully assigned ${res.assigned} lead(s) to ${formatAssignStaffSummary(assignSelectedStaffList)}!`
      )
      resetAssignSelectedDirty()
      await refreshData()
      setSelectedIds(new Set())
      setTimeout(() => {
        setAssignSelectedSuccessMessage('')
        setAssignSelectedStaffList([])
  ...
  return (
    <Layout>
                ...
                <div className="flex items-center gap-1.5 pl-2 sm:border-l sm:border-slate-200">
                  ...
                  <div className="relative">
                    <button
                      ...
                      className={`flex w-44 items-center justify-between gap-2 rounded-lg border bg-slate-50/80 px-2.5 py-1.5 text-xs font-medium text-slate-700 transition hover:bg-slate-100 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 cursor-pointer ${
                        sourceDropdownOpen ? 'border-brand-500 ring-1 ring-brand-500' : 'border-slate-200'
                      }`}
                    >
                    ...
                    {sourceDropdownOpen && (
                      <>
                        ...
                        />
                        <div className="absolute left-0 z-20 mt-1.5 max-h-72 min-w-full overflow-y-auto whitespace-nowrap rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl">
                          <button
                            type="button"
                            onClick={() => {
                              setSelectedSource('All Sources')
                            ...
                            className="block w-full rounded-lg px-3 py-2 text-left text-xs font-semibold text-slate-700 hover:bg-slate-50 cursor-pointer"
                          >
                            All Sources
                          </button>
                          {sourceOptions.map((src) => (
                            <button
                              ...
                              className="block w-full rounded-lg px-3 py-2 text-left text-xs text-slate-700 hover:bg-slate-50 cursor-pointer"
                            >
                              {src.name}
                            </button>
                          ))}
      ...
      {/* Assign Selected Leads to Staff Modal (Managers and above only) */}
      {assignSelectedOpen && (
        <div
          className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/60 backdrop-blur-xs p-0 sm:p-4"
          onClick={(e) => {
            if (e.target === e.currentTarget && !assignSelectedDirty) setAssignSelectedOpen(false)
          }}
        >
          <div className="flex max-h-full w-full flex-col overflow-hidden rounded-t-2xl sm:rounded-2xl bg-white shadow-2xl border border-slate-100 animate-in fade-in zoom-in-95 duration-150 sm:max-w-xl">
            {/* Modal Header */}
            <div className="flex shrink-0 items-center justify-between gap-3 px-5 sm:px-6 py-4 border-b border-slate-100 bg-slate-50/70">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h3 className="text-base font-bold text-slate-900">
                    Assign Selected Leads to Staff
                  </h3>
                  <p className="text-xs text-slate-500">
                    Distribute the selected raw leads across sales and telecalling teams.
                  </p>
                </div>
              </div>
              <button
                type="button"
                onClick={requestCloseAssignSelected}
                aria-label="Close"
                className="shrink-0 rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition cursor-pointer"
              >
                <CloseIcon />
            ...
            <form onSubmit={handleExecuteAssignSelected} className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4">
              {/* Step 1: Select Staff Member */}
              <div className="rounded-xl border border-slate-200 bg-white p-4 space-y-2.5 shadow-2xs">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-medium text-slate-500">
                    {assignSelectedStaffList.length === 0
                      ? 'None selected'
                      : `${assignSelectedStaffList.length} selected`}
                  </span>
                </div>
                ...
                <div className="relative mt-1">
                  ...
                  <button
                    type="button"
                    onClick={() => setAssignSelectedStaffOpen((v) => !v)}
                    className={`flex w-full items-center justify-between gap-2 rounded-lg border bg-slate-50/50 px-3 py-2 text-left text-xs transition cursor-pointer ${
                  ...
                  >
                    <span className={`truncate font-semibold ${
                      assignSelectedStaffList.length === 0 ? 'text-slate-400' : 'text-slate-800'
                    }`}>
                      {assignSelectedStaffList.length === 0
                        ? 'Select staff members…'
                  ...
                  {assignSelectedStaffOpen && (
                    <>
                      <div
                        className="fixed inset-0 z-10"
                        onClick={() => setAssignSelectedStaffOpen(false)}
                      />
                      <div className="absolute left-0 right-0 z-20 mt-1.5 max-h-56 overflow-y-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl">
                        {assignableStaff.length === 0 ? (
                          <p className="px-3 py-2 text-xs text-slate-400">No assignable staff</p>
                          ...
                          [
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={
                                  assignSelectedStaffList.length === assignableStaff.length && assignableStaff.length > 0
                                }
                                onChange={toggleAllAssignSelectedStaff}
              ...
              <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2.5 pt-2 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={isSaving || assignSelectedStaffList.length === 0 || selectedIds.size === 0}
                  className="flex items-center justify-center gap-1.5 rounded-xl bg-emerald-600 px-5 py-2.5 text-xs font-bold text-white shadow-md shadow-emerald-600/20 hover:bg-emerald-700 transition active:scale-[0.98] cursor-pointer disabled:cursor-not-allowed disabled:opacity-50"
                >
                  <span>✓ Assign {selectedIds.size} Selected Lead(s) to {assignSelectedStaffList.length > 0 ? `${assignSelectedStaffList.length} Staff` : 'Select Staff'}</span>
                </button>
              </div>
            </form>
          </div>
      ...
      />
      <ConfirmDialog
        open={discardAssignSelectedOpen}
        onCancel={() => setDiscardAssignSelectedOpen(false)}
        onConfirm={() => {
          setDiscardAssignSelectedOpen(false)
          setAssignSelectedOpen(false)
          resetAssignSelectedDirty()
        }}
      />
