...
const EMPTY = {
  ...
  trend: [],
  sources: [],
  funnel: { total: 0, contacted: 0, interested: 0, quotation: 0, order: 0 },
  pipeline: [],
  team: [],
  hot_leads_list: [],
}
...
export default function Dashboard() {
  ...
  const [period, setPeriod] = useState('last30') // 'today' | 'last7' | 'last30' | 'last365' | 'custom'
  ...
  // Resolve the actual date range for the current selection. Points are per-day
  // (daily buckets) unless the custom range is very large, in which case monthly
  // buckets keep the chart readable.
  const resolved = useMemo(() => {
    ...
    if (period === 'custom') {
      if (customFrom && customTo && customTo >= customFrom) {
        ...
        return {
          groupBy: spanDays >= 366 ? 'monthly' : 'daily',
          start: toISODate(s),
          end: toISODate(e),
        }
      }
      // Fallback to the last 30 days when custom is incomplete.
  ...
  const kpiDefs = useMemo(
    () => [
      ...
      { label: 'Orders Accepted', key: 'orders_accepted', trend: 'up', change: '', tone: 'green', icon: 'M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z' },
    ],
    []
  )
  ...
  const kpis = useMemo(
    () =>
      kpiDefs.map((def) => ({
        ...def,
        value: (stats.kpis[def.key] ?? 0).toLocaleString(),
      })),
    [kpiDefs, stats]
  )

  // Conversion funnel derived from real data.
  const conversionData = useMemo(() => {
    const f = stats.funnel || EMPTY.funnel
    const total = f.total || 0
    const rows = [
      { name: 'Total Leads', value: total, color: '#3b82f6' },
      { name: 'Contacted', value: f.contacted || 0, color: '#6366f1' },
      { name: 'Interested', value: f.interested || 0, color: '#8b5cf6' },
      { name: 'Quotations', value: f.quotation || 0, color: '#f59e0b' },
      { name: 'Orders Accepted', value: (f.order || 0) + (f.client || 0), color: '#10b981' },
    ]
    return rows.map((r) => ({ ...r, rate: total ? `${((r.value / total) * 100).toFixed(1)}%` : '0%' }))
  }, [stats])

  const sourceTotal = useMemo(
    () => (stats.sources || []).reduce((acc, s) => acc + s.value, 0),
    [stats]
  )
  ...
  const periodLabel =
    period === 'custom'
      ? `${customFrom || resolved.start} – ${customTo || resolved.end}`
      : presetLabels[period]
  ...
  return (
    <Layout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Unified Lead-to-Order sales pipeline and operational overview • {periodLabel}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            {period === 'custom' && (
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  className="rounded-lg border border-slate-200 bg-slate-50/80 px-2 py-1.5 text-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500 cursor-pointer"
                />
            ...
            <select
              ...
              className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 cursor-pointer"
            >
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-24 text-slate-400">
            <div className="h-8 w-8 animate-spin rounded-full border-2 border-slate-200 border-t-brand-600" />
            <p className="mt-3 text-sm">Loading dashboard data…</p>
          </div>
        ) : error ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
              ...
              <ChartCard
                ...
                subtitle="Inbound leads vs contacted"
                badge={periodLabel}
                ...
                <div className="h-64 text-[11px]">
                  {stats.trend.length ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={stats.trend} margin={{ top: 10, right: 10, left: -18, bottom: 0 }}>
                        <defs>
              ...
              <ChartCard
                ...
                badge={`${stats.sources.length} Channels`}
              ...
              >
                {stats.sources.length ? (
                  <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-center">
                    ...
                    <div className="w-full max-h-52 overflow-y-auto space-y-2.5 pr-1 thin-scroll">
                      {stats.sources.map((s) => (
                  ...
                  <div className="flex h-full items-center justify-center text-sm text-slate-400">No source data for this period</div>
                )}
              </ChartCard>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 text-[11px] text-slate-400">
                        {conversionData[0].value.toLocaleString()} leads → {conversionData[4].value.toLocaleString()} orders accepted
                      </p>
                    </div>
                  </div>
              ...
              <div className="rounded-2xl border border-brand-200/90 bg-white p-6 shadow-sm lg:col-span-5">
                ...
                {stats.hot_leads_list.length ? (
                  <div className="space-y-3">
                    {stats.hot_leads_list.map((lead) => (
                      <div
                        key={lead.id}
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
              ...
              <ChartCard
                ...
                subtitle="Quotations vs orders accepted"
                badge={periodLabel}
                className="lg:col-span-7"
              >
                <div className="h-64 text-[11px]">
                  {stats.pipeline.length ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={stats.pipeline} margin={{ top: 10, right: 10, left: -18, bottom: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false} />
                        <XAxis dataKey="label" tickLine={false} axisLine={false} tick={{ fontSize: 11 }} />
                        <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 11 }} />
                        <Tooltip contentStyle={TOOLTIP_STYLE} />
              ...
              <ChartCard
                ...
                badge={`${stats.team.length} staff`}
                className="lg:col-span-5"
              >
                <div className="h-64 text-[11px]">
                  {stats.team.length ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={stats.team} layout="vertical" margin={{ top: 5, right: 10, left: 10, bottom: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#f1f5f9" />
                        <XAxis type="number" hide />
                        <YAxis
                          dataKey="name"
