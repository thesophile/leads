...
export default function Branch() {
  const { user } = useAuth()
  const canManage = !!user && can(user, 'branch.manage')
            ...
            <form onSubmit={handleSave} className="mt-3 space-y-3">
              ...
              {canManage ? (
                ...
                <p className="pt-1 text-[11px] text-slate-400">
                  View only — you need the &ldquo;branch.manage&rdquo; permission to add or edit branches.
                </p>
              )}
            </form>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-xs lg:col-span-8">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-4">
              ...
              <div className="flex items-center gap-2">
                <RefreshButton onClick={refreshData} loading={isLoading} compact title="Refresh branches" />
                ...
                <tbody className="divide-y divide-black">
                  {paginatedBranches.length > 0 ? (
                    paginatedBranches.map((br) => (
                      <tr
                        key={br.id}
                        onClick={() => { if (canManage) handleEditClick(br) }}
                        ...
                        {canManage && (
                          <td className="py-0.5 pr-2 text-right">
                            <div className="flex items-center gap-1.5 justify-end">
                              <button
                                type="button"
                                onClick={(e) => { e.stopPropagation(); handleEditClick(br) }}
                                title="Edit Branch"
