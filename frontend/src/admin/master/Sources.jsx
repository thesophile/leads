...
export default function Sources() {
  const { user } = useAuth()
  const canManage = !!user && can(user, 'source.manage')
  ...
  async function handleSave(e) {
    e.preventDefault()
    if (isLoading || !canManage || saving) return
    setError('')
          ...
          <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-xs lg:col-span-4 transition-all">
            ...
            {isLoading && (
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-white/85 backdrop-blur-xs transition-opacity">
                <SpinnerIcon />
                <span className="mt-2 text-xs font-semibold text-slate-600">Loading source data...</span>
              </div>
            )}
            ...
            <form onSubmit={handleSave} className="mt-3 space-y-3">
              {/* Source Name Floating Label Input */}
              ...
              {canManage ? (
                ...
                <p className="pt-1 text-[11px] text-slate-400">
                  View only — you need the &ldquo;source.manage&rdquo; permission to add or edit sources.
                </p>
              )}
            </form>
          </div>

          {/* Right Source List Card */}
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-xs lg:col-span-8">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-4">
              <div className="flex items-center gap-2.5">
                <TableIcon />
                <h2 className="text-base font-bold text-slate-900">Source List</h2>
              </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-black text-slate-800 font-bold uppercase tracking-wider text-[11px]">
                    ...
                    <th className="pb-2 font-semibold w-72">Source Name</th>
                ...
                <tbody className="divide-y divide-black">
                  {paginatedSources.length > 0 ? (
                    paginatedSources.map((src) => (
                      <tr
                        key={src.id}
                        onClick={() => { if (canManage) handleEditClick(src) }}
                        ...
                        {canManage && (
                          <td className="py-0.5 pr-2 text-left">
                            <div className="flex items-center gap-1.5">
                              <button
                                type="button"
                                onClick={(e) => { e.stopPropagation(); handleEditClick(src) }}
                                title="Edit Source"
            ...
            <div className="flex items-center justify-between pt-3 mt-3 border-t border-slate-100 text-[11px]">
              <span className="text-slate-400 font-medium">
                Showing{' '}
                {filteredSources.length > 0 ? (currentPage - 1) * pageSize + 1 : 0} to{' '}
                {Math.min(currentPage * pageSize, filteredSources.length)} of{' '}
      ...
      {deleteModalId && (
        ...
        >
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl border border-slate-200">
            <h3 className="text-base font-bold text-slate-900">Delete Source</h3>
